# OKD Virtualization

[![Build Status](https://prow.ci.openshift.org/badge.svg?jobs=periodic-ci-okd-virtualization-release-master-okd-e2e-deploy)](https://prow.ci.openshift.org/job-history/gs/origin-ci-test/logs/periodic-ci-okd-virtualization-release-master-okd-e2e-deploy)

# Testing the integration between

- [OKD](https://github.com/openshift/okd)
- [HCO](https://github.com/kubevirt/hyperconverged-cluster-operator) / [KubeVirt](https://github.com/kubevirt/kubevirt)

## In future
- [NHC](https://github.com/medik8s/node-healthcheck-operator)
- [Rook](https://github.com/rook/rook)
- [Konveyor](https://github.com/orgs/konveyor)

# Job History
- [okd-e2e-deploy](https://prow.ci.openshift.org/job-history/gs/origin-ci-test/logs/periodic-ci-okd-virtualization-ci-master-okd-e2e-deploy)
